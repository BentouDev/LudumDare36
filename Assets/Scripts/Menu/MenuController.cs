...
public class MenuController : GUIBase
{
    ...
    protected IEnumerator DoShowAnim(MenuBase targetMenu = null)
    {
        var menu = targetMenu ?? TitleMenu ?? MainMenu;
    ...
    public void AnimShow(MenuBase targetMenu = null)
    {
        if (IsDuringShowing || IsDuringHiding)
            return;

        StartCoroutine(DoShowAnim(targetMenu));
    }
